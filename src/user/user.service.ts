...
import { Habitat } from '../habitat/interfaces/habitat.interface';
...
export class UserService {
  ...
  async getUsersBySession(idSession: string): Promise<Array<User>> {
    let habitats = await this.habitatModel.find({
    ...
    var idClients = [];
    for (var n = 1; n < habitats.length; ++n) {
      habitats.forEach((habitat) => {
        idClients.push(habitat.idClient);
      });
    }
